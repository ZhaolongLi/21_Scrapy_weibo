...
class WeibocnSpider(Spider):
    ...
    fan_url = 'https://m.weibo.cn/api/container/getIndex?container=231051_-_fans_-_{uid}&since_id={page}'
    ...
    def parse_user(self,response):
        ...
        if result.get('data').get('userInfo'):
            ...
            field_map = {
                ...
                'follows_count':'follow_count','weibos_count':'status_count','verified':'verified',
    ...
    def parse_fans(self,response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) \
            ...
            yield Request(self.fan_url.format(uid=uid,page=page),
                          callback=self.parse_fans,meta={'since_id':page,'uid':uid})
